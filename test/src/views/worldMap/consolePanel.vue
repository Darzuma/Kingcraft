<template>
    <div class="consolePanel">
        ...
        <div class="modify" @click="$store.user.showHeroes = true"/>
        <div class="crystal">
            {{ hero.level + 2 }}
        </div>
        <div class="heroName">
...
<!--                <i class="level">{{ card.level }}</i>-->
        ...
        <div class="repositoryTips" v-show="currentRepoCard">
            <i class="tips">{{ currentRepoCard ?
                (currentRepoCard.valid ? currentRepoCard.equiped ? 'Click to unequip' : 'Click to equip' : `Unlock [${currentRepoCard.requirement ? currentRepoCard.requirement : $store.heroes[currentRepoCard.col - 2].name}] to to get this card`) : '' }}</i>
...
export default {
    ...
    computed:{
        ...
        cards(){ return this.$store.cards },
        hero(){
            return this.$store.heroes[this.user.heroIndex]
        },
        index(){
            return this.user.heroIndex
        },
        slots(){
            let cards = this.cards
            let arr = []
            this.user.slots.sort((a,b)=> a - b)
            this.user.slots.forEach(index => {
    ...
    methods:{
        ...
        equip(card, index){
            let user = this.user
            if(!card.valid){
                this.user.heroesDialogVM.index = card.heroIndex || typeof card.heroIndex === 'number' ? card.heroIndex : card.col - 2
                this.user.showHeroes = true
            }
            else{
        ...
        unequip(card, index){
            card.equiped = false
            this.user.slots.splice(this.user.slots.indexOf(card.id), 1)
